...
static t_cmd	*handle_redirections(t_cmd *ast, t_alloc *heap)
{
	...
	if (file_fd == -1)
		(perror(ast->file), cleanup(heap), rl_clear_history(), exit(1));
	if (dup2(file_fd, ast->fd) == -1)
		(perror("dup2"), cleanup(heap), rl_clear_history(), exit(1));
	close(file_fd);
	return (exec_node);
}
...
void	execute_child_task(t_cmd *node, t_alloc *heap)
{
	...
	if (fullpath == NULL)
		(cleanup(heap), rl_clear_history(), exit(127));
	if (execve(fullpath, exec_node->argv, heap->ev_clone) == -1)
		(perror(fullpath), free(fullpath), cleanup(heap), rl_clear_history(), exit(126));
}
...
static void	execute_single_command(t_cmd *ast, t_alloc *heap)
{
	...
	pid = fork();
	if (pid == -1)
		(perror("fork"), cleanup(heap), rl_clear_history(), exit(1));
	if (pid == 0)
...
static void	execute_pipe(t_cmd *ast, t_alloc *heap)
{
	...
	if (pipe(pipefd) == -1)
		(perror("pipe"), cleanup(heap), rl_clear_history(), exit(1));
	pid_left = fork();
	if (pid_left == -1)
		(perror("fork"), cleanup(heap), rl_clear_history(), exit(1));
	if (pid_left == 0)
